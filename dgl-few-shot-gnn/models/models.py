...
from models import dgl_gnn_iclr
...
class MetricNN(nn.Module):
    def __init__(self, args, emb_size):
        ...
        if self.metric_network == 'gnn_iclr_nl':
            ...
            num_inputs = self.emb_size + self.args.train_N_way # 128+5
            if self.args.dataset == 'mini_imagenet':
                self.gnn_obj = dgl_gnn_iclr.GNN_nl(args, num_inputs, nf=96, J=1) #J=1????????????????
            elif 'omniglot' in self.args.dataset:
                self.gnn_obj = dgl_gnn_iclr.GNN_nl_omniglot(args, num_inputs, nf=96, J=1)
        elif self.metric_network == 'gnn_iclr_active':
            assert(self.args.train_N_way == self.args.test_N_way)
            num_inputs = self.emb_size + self.args.train_N_way
            self.gnn_obj = dgl_gnn_iclr.GNN_active(args, num_inputs, 96, J=1)
        else:
            raise NotImplementedError

    def gnn_iclr_forward(self, z, zi_s, labels_yi):
        # Creating WW matrix ,train_batch_size=40,test_batch_size=10
        # z(tensor):batch_size,128 待测样本embedding
        # zi_s(list): 25 已知样本embedding
        # labels_yi(list):25, 已知样本label
        # labels_yi[0].size()：[batch_size,5]
        zero_pad = Variable(torch.zeros(labels_yi[0].size())) 
        if self.args.cuda:
            zero_pad = zero_pad.cuda()

        labels_yi = [zero_pad] + labels_yi #把待测的和已知的放在一起 26
        zi_s = [z] + zi_s

        nodes = [torch.cat([zi, label_yi], 1) for zi, label_yi in zip(zi_s, labels_yi)] #26,(40,133)
        nodes = [node.unsqueeze(1) for node in nodes] #26,(40,1,133)
