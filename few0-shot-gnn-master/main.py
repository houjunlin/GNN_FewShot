...
parser.add_argument('--active_random', type=int, default=0, metavar='N',
                    help='random active ? ')
parser.add_argument('--dataset_root', type=str, default='/home/hjl/fewshot/few-shot-gnn-master/datasets', metavar='N',
                    help='Root dataset')
...
def train_batch(model, data):
    ...
    [batch_x, label_x, batches_xi, labels_yi, oracles_yi, hidden_labels,labels_mask] = data
    ...
    out_metric, out_logits = metric_nn(inputs=[z, zi_s, labels_yi, oracles_yi, hidden_labels,labels_mask])
...
def train():
    ...
    for batch_idx in range(args.iterations):
        ...
        [batch_x, label_x, _, _, batches_xi, labels_yi, oracles_yi, hidden_labels, labels_mask] = data

        opt_enc_nn.zero_grad()
        opt_metric_nn.zero_grad()

        loss_d_metric = train_batch(model=[enc_nn, metric_nn, softmax_module],
                                    data=[batch_x, label_x, batches_xi, labels_yi, oracles_yi, hidden_labels, labels_mask])
