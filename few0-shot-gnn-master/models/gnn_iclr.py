...
def gmul(input,labels_mask):
    ...
    W = W * labels_mask.unsqueeze(3).repeat(1, 1, 1, W.size()[-1])
...
class Gconv(nn.Module):
    ...
    def forward(self, input, labels_mask):
        W = input[0]
        x = gmul(input,labels_mask) # out has size (bs, N, num_inputs)
        #if self.J == 1:
        #    x = torch.abs(x)
        x_size = x.size()
        x = x.contiguous()
...
class GNN_nl(nn.Module):
    ...
    def forward(self, x, labels_mask):
        W_init = Variable(torch.eye(x.size(1)).unsqueeze(0).repeat(x.size(0), 1, 1).unsqueeze(3))
        if self.args.cuda:
            W_init = W_init.cuda()
        ...
        for i in range(self.num_layers):
            ...
            x_new = F.leaky_relu(self._modules['layer_l{}'.format(i)]([Wi, x],labels_mask)[1])
            x = torch.cat([x, x_new], 2)

        Wl=self.w_comp_last(x, W_init)
        out = self.layer_last([Wl, x],labels_mask)[1]
