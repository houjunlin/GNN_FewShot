...
class Generator(data.Dataset):
    ...
    def get_task_batch(self, batch_size=5, n_way=20, num_shots=1, unlabeled_extra=0, cuda=False, variable=False):
        ...
        #print(len(labels_yi)) 25
        #print(labels_yi[0].size()) 40,5
        labels_mask = torch.ones(batch_size, num_shots * n_way + 1, num_shots * n_way + 1)
        for bs in range(batch_size):
            for i in range(len(labels_yi)):
                for j in range(len(labels_yi)):
                    for n in range(n_way):
                        #print(labels_yi[i][bs],labels_yi[j][bs])
                        if not labels_yi[i][bs][n] == labels_yi[j][bs][n]:
                            labels_mask[bs,i+1,j+1] = 0
        ...
        return_arr = [torch.from_numpy(batch_x), torch.from_numpy(labels_x), torch.from_numpy(labels_x_scalar),
                      ...
                      torch.from_numpy(hidden_labels),labels_mask]
        if cuda:
